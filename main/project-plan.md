...
- [~] Set Resend API key (user secret) and admin notification — IN PROGRESS: subdomain sender chosen (mail.haikaitech.my -> noreply@mail.haikaitech.my), RESEND_FROM_EMAIL updated in wrangler.jsonc; BLOCKED: user must add Resend DNS records, verify domain in Resend, and provide API key
...
- [ ] Build clean, Lighthouse >= 95 on toolkit page
...
2026-08-03 - Resend setup in progress: confirmed haikaitech.my runs Cloudflare Email Routing (MX route*.mx.cloudflare.net + SPF include:_spf.mx.cloudflare.net), chose subdomain sender noreply@mail.haikaitech.my (avoids touching apex SPF), RESEND_FROM_EMAIL var updated in wrangler.jsonc. Awaiting user: add Resend DNS records (resend._domainkey.mail TXT, send.mail MX + TXT, links CNAME), verify mail.haikaitech.my in Resend, confirm Email Routing destination for admin@haikaitech.my, provide Resend API key.
